...
// For Android emulator: 10.0.2.2 is the host machine (requires local backend running)
// For iOS simulator: localhost or 127.0.0.1
// For physical device: use actual IP or production URL
// NOTE: Using production backend by default. For local development:
//   1. Ensure backend is running on port 5000
//   2. Set EXPO_PUBLIC_API_URL=http://10.0.2.2:5000/api in .env.local
const getAPIUrl = () => {
  if (process.env.EXPO_PUBLIC_API_URL) {
    return process.env.EXPO_PUBLIC_API_URL;
  }

  // Default to production API (both Android and iOS)
  const baseUrl = "https://expo-ecommerce-three.vercel.app/api";
